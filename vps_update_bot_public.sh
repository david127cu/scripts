echo Destruyendo el contenedor public en el VPS:
docker rm public --force
echo Destruyendo la imagen docker previa del bot en el VPS:
docker rmi botcito_public
...
docker image build -t botcito_public .
cd #
...
ls
docker run --restart always --interactive --name public botcito_public